...
$codigo_idea = $url_pagina[1];

$consulta = "SELECT * FROM anteproyectos where codigo_anteproyecto = :datos";

$consulta = $ins_loginControlador->consulta_information($codigo_idea, $consulta);

if ($consulta->rowCount() == 1) {

    // Obtener los datos de la idea
    $idea = $consulta->fetch(PDO::FETCH_ASSOC);

    $id_proyecto = $idea['id_anteproyecto'];

    $codigo = $idea['codigo_anteproyecto'];

    $titulo = $idea['titulo_anteproyecto'];

    $palabrasClaves = $idea['palabras_claves'];

    $estado_anteproyecto= $idea['estado'];
...
WHERE ae.codigo_anteproyecto = :codigo";
...
if ($sql->rowCount() > 0) {
    ...
    if(isset($estado_anteproyecto)){
        if($estado_anteproyecto == "Cancelado"){
            echo '<div class="alert alert-danger" role="alert">
            <div class="text-center">El anteproyecto ha sido cancelado</div>
            </div>';
            ?>
            <form class="user-form mt-5 mb-5 FormulariosAjax" action="<?= SERVERURL ?>Ajax/AnteproyectoAjax.php" method="POST" data-form="update" autocomplete="off">
                <h2><i class="fa-solid fa-user-pen"></i> Información idea anteproyecto</h2>

                
                <div class="form-grid two">
                    <input type="hidden" name="codigo_idea_upd_estado" value="<?= $ins_loginControlador->encryption($codigo) ?>" readonly>

                    <div class="form-floating">
                        <input type="text" class="form-control input_border" id="floatingNombre" name="titulo_idea_upd"
                            placeholder="Password" value="<?= $titulo ?>">
                        <label for="floatingNombre mb-4">Titulo idea</label>
                    </div>

                    <select class="form-select" name="actualizar_estado_anteproyecto" aria-label="Default select example">
                ...
                <div class="form-actions mt-5 mb-5">
                    <button type="submit"><i class="fa-solid fa-pen"></i> &nbsp; Actualizar idea</button>
                </div>
            </form>
            <?php
        }
        ...
        <tr>
            ...
            <td>
                <form class="FormulariosAjax" action="<?php echo SERVERURL; ?>Ajax/AnteproyectoAjax.php" method="POST" autocomplete="off" data-form="delete">
                    <input type="hidden" name="documento_userDA" value="<?php echo $encriptado; ?>">
                    <input type="hidden" name="codigoDA" value="<?php echo $ins_loginControlador->encryption($codigo) ?>">
    ...
    <div class="text-center">No se encontraron usuarios asignados a la idea de anteproyecto</div>
    </div>';
}
...
?>
    ...
    <form class="user-form mt-5 mb-5 FormulariosAjax" action="<?= SERVERURL ?>Ajax/AnteproyectoAjax.php" method="POST" data-form="update" autocomplete="off">
        ...
        <div class="form-grid two">
            <input type="hidden" name="codigo_idea_upd" value="<?= $ins_loginControlador->encryption($codigo) ?>" readonly>
            ...
            <div class="form-floating">
                <input type="text" class="form-control input_border" id="floatingNombre" name="palabras_claves_upd"
    ...
    if(isset($codigo)){
        ...
        if ($resultado_consulta_asesor->rowCount() > 0) {
            ...
            if (!empty($usuario['numero_documento']) && $usuario['numero_documento'] !== null) {
                 // Construir la tabla con Bootstrap
             echo '<div class="table-responsive mt-2 mb-2">
                    ...
                    <div class="card card-body  actualizar_asesor_anteproyecto">
                      ...
                      <form class="user-for  mb-5 FormulariosAjax" action="<?= SERVERURL ?>Ajax/UsuarioAjax.php" method="POST" data-form="save" autocomplete="off">
                        ...
                        <div class="form-grid">
                            ...
                            <div class="form-floating">
                            <input type="number" class="form-control input_border" id="floatingDocumento" name="documento_asesor_actualizar" placeholder="Numero de documento" onchange="BuscarUsuario(this.value, '<?= SERVERURL ?>')">
                            <label for="floatingDocumento">Numero de documento</label>
                            </div>
                            <input type="hidden" name="codigo_proyecto" value="<?= $ins_loginControlador->encryption(1) ?>" readonly>
