...
if (isset($_GET['views'])) {
  $ruta = explode(
   ...
   $codigo_proyecto_recibido = $ruta[1];
...
<form class="user-form mt-5 mb-5 FormulariosAjax" action="<?= SERVERURL ?>Ajax/ProyectoAjax.php" method="POST" data-form="save" autocomplete="off">
  ...
  <h2><i class="fas fa-book"></i> Información básica profesores y proyectos</h2>
...
if (isset($codigo_proyecto_recibido) && !empty($codigo_proyecto_recibido)) {
  ...
  WHERE ajp.codigo_proyecto = '$codigo_proyecto_recibido'";
  ...
  if ($resultado_verificar_jurado_proyecto->rowCount() > 0) {
    ...
    echo 'Este es el código del proyecto para asignar jurados: <span class="badge bg-success">' . $codigo . '</span>';
    ...
    echo '</div>';
      ...
      echo '<div class="alert alert-danger text-center" role="alert">
        ...
        echo 'Este es el código del proyecto para asignar jurados: <span class="badge bg-success">' . $codigo_proyecto_recibido . '</span>';
...
?>
  ...
  <div class="form-grid two mt-5">
    <div class="form-floating">
      <input type="number" class="form-control input_border" id="codigo_proyecto_asignar"
        name="codigo_proyecto_asignar_jurado" placeholder="Codigo anteproyecto"  onchange="BuscarProyecto(this.value, '<?= SERVERURL ?>')">
      <label for="codigoAnteproyecto">Código proyecto </label>
    </div>
    <div class="form-floating">
      <input type="text" class="form-control input_border" id="documento_user_asignar"
        name="documento_user_asignar_jurado" placeholder="Numero de documento" onchange="BuscarUsuario(this.value, '<?= SERVERURL ?>')">
      <label for="numeroDocumento">Numero de documento director</label>
    </div>
  ...
  <div class="form-actions mt-5 mb-5">
    <button type="submit"><i class="fa-regular fa-floppy-disk"></i> &nbsp; Asignar director proyecto o anteproyecto</button>
  </div>
</form>
